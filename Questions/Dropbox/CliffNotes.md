...
4. Clients will chunk the file. In JavaScript then can do `file.slice` and in Java then can do `RandomAccessFile`. These methods do not copy the entire file into memory. They read and upload the selected range as needed. Chunking also allows us to upload parts in parallel if there is available bandwidth
...
7. Before we call `CompleteMultipartUpload` it is better to also check if the parts are not corrupted. Every part uploaded receives an `eTag`. We send the `eTag` to the application for persistence in DB after marking the chunk upload as complete. The application calls `ListParts` to get all the parts that were uploaded to S3. The response contains the `eTag` which the application server can use to verify. 

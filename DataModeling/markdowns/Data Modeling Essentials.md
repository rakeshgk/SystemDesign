...
- **Primary keys — UUID vs bigint:** auto-incrementing `bigint` keys are compact and index-friendly but leak volume and growth rate when exposed externally. They also require a round-trip to the DB to generate. UUIDs (or ULIDs) can be generated client-side and don't leak counts, but they're larger (16 bytes) and, if random (UUIDv4), hurt index locality on inserts. ULIDs and UUIDv7 are time-ordered and avoid that penalty — a good default when you need client-generated IDs — though both embed, and therefore leak, the record's creation timestamp. Store them as the DB's native 16-byte type (UUID in Postgres) rather than the 26-char string: it's smaller and gives faster index comparisons, and the conversion is lossless since both are 128 bits.
...
- **Composite indexes and column order.** An index on `(user_id, created_at)` efficiently serves "posts by user, most recent first" but does not help a query that filters on `created_at` alone. Think of a phone book sorted by last name, then first name: finding everyone named "Smith" is fast, and "Smith, John" is fast too — but finding everyone whose first name is "John" forces a full scan, because first names aren't globally ordered. A composite index works the same way: only left-to-right prefixes of the columns are usable. So put the equality-filter column first and the range/sort column second.

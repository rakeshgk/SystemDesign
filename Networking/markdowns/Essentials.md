...
4. **Security groups and network ACLs**: These are the two layers of firewall rules that decide which traffic is allowed, and the difference between them is a favorite interview check.
   - A **security group** attaches to an *instance* (or a load balancer, database, etc.) and acts as its personal firewall. It is **stateful**: if you allow an inbound request, the response is automatically allowed back out — you don't write a separate rule for the return traffic. Security groups are also **allow-only** (you list what's permitted; everything else is denied) and can reference *other security groups* rather than raw IP ranges. Example: the database's security group says "allow inbound TCP 5432 **only from the app-tier security group**," and the app tier's says "allow inbound TCP 443 only from the load balancer's security group." No IP addresses hard-coded — instances are authorized by *role*.
   - A **network ACL** attaches to a *subnet* and filters traffic crossing that subnet's boundary. It is **stateless**: it evaluates each packet in isolation, so you must explicitly allow *both* directions (e.g. inbound 443 **and** the outbound ephemeral port range for the responses). NACLs support explicit **deny** rules, which security groups can't — useful for blocklisting a specific bad IP range. Example: a NACL on a private subnet denies all inbound traffic from the public internet as a coarse safety net, regardless of what any individual instance's security group might allow.
   - The mental model: security groups are the fine-grained, role-based lock on each door (stateful, allow-only); network ACLs are the coarse guard at the subnet perimeter (stateless, allow + deny). Together they enforce "only the load balancer can talk to the app tier, only the app tier can talk to the database" in **defense in depth** — a request has to pass both layers.

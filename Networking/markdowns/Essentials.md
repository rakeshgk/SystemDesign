...
4. **Security groups and network ACLs**: Firewall rules that control which traffic is allowed. Security groups are stateful and attach to instances; network ACLs are stateless and attach to subnets. This is where you enforce "only the load balancer can talk to the app tier, only the app tier can talk to the database."
